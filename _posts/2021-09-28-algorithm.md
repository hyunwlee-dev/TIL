...
3. 자바를 주언어로 하면서 Collection안을 들여다 본적이 있나? => X
...
> <span style="background-color:lightgreen">__DS - Data Structure__</span>
...
<span style="background-color:lightgray">자료구조 선택문제는 대개 `추상 자료형`의 선택으로부터 시작하는 경우가 많다.</span>
...
- push, pop operation이 있다. stack 사이즈 operation, stack 꼭대기에 있는 확인 operation등이 있다.
...
그럼 실제로 스택을 구현하게 되면 어떻게 구현을 할것인지 말을 하면 => Data Structure가 되는 것
```
...
__예를 들어) 월드컵에서 골을 넣은 선수를 관리하는 자료구조? 알파벳 순서로 하고 싶음__

__ADT__:  Alphabetical Ordered
...
> <span style="background-color:lightgreen">분류</span>

__형태에 따른 자료구조__

__1. 선형 자료구조(Linear Data Structure)__
...
__2. 선형 자료구조(NonLinear Data Structure)__

일렬로 나열된 것이 아닌, 각 요소가 여러 개의 요소와 연결 된 형태를 생각하면 된다.

__Graph__, __Tree__가 있다.



__3. 집합 자료구조__

두 가지 분류에 속하지 않는 __set__ 자료구조가 있다.
...
__Java Collections__ + __Framework__

일정 타입의 데이들이 모여 쉽게 가공할 수 있도록 지원하는 자료구조들의 뼈대가 되는 기본 구조



자바에서 기본 구조라 한다면 ADT __Interface__가 떠올라야 한다.
...
하지만 Iterable에서는 인터페이스를 보면 알겠지만 __for-each__를 제공한다.
...
범위를 넘어서게 되면 __IndexOutOfBoundsException__라는 에러가 발생한다.

이러한 단점을 보완하여 List를 통해 구현된 클래스들은 '동적 크기'를 갖으며 배열처럼 사용할 수 있게 된다.

__배열의 기능 + 동적 크기 할당__이 합쳐져 있다고 보면 된다.
...
__Object[]__ 배열을 사용하면서 내부 구현을 통해 동적으로 관리한다.
...
__데이터(item)__와 주소로 이루어진 클래스를 만들어 서로 연결하는 방식이다.

데이터와 주소로 이루어진 클래스를 Node(노드)라고 하는데, 각 노드는 __노드(prev)__와 __노드(next)__를 연결하는 방식인 것이다. 즉, 이중연결 리스트이다.
...
__동기화__를 지원한다. 쓰레드가 동시에 데이터에 접근하려하면 순차적으로 처리하도록 한다. 그렇다보니 멀티 쓰레드에서는 안전하지만, 단일 쓰레드에서도 동기화를 하기 때문에 ArrayList에 비해 성능이 약간 느리다.
...
> <span style="background-color:lightgreen">Queue/Deque Interface Operations</span>
...
> <span style="background-color:lightgreen">Queue/Deque Interface를 구현하는 클래스</span>
...
__원리 자체가 크게 다르지 않기 때문에 linkedList 하나에 다중 인터페이스를 포함하고 있는 것이다.__
...
> <span style="background-color:lightgreen">Set Interface Operation</span>
...
<span style="background-color:lightgreen">Set Interface를 구현하는 클래스</span>
...
Set Interface를 상속받은 __SortedSet Interface__를 구현하고 있다.

__가중치에 따른 순서대로 정렬__되어 보장한다는 것이다.
...
[블로그 - st-lab](https://st-lab.tistory.com/142){:target="_blank"}

[유튜브 - 쉬운코드](https://www.youtube.com/watch?v=QcsQKgXemtA&t=9s){:target="_blank"}
